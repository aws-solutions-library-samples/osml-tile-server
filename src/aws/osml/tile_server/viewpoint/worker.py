...
from .models import ViewpointStatus, ViewpointModel
...
class ViewpointWorker():
    def __init__(self, viewpoint_request_queue: ViewpointRequestQueue, aws_s3: ServiceResource, viewpoint_database: ViewpointStatusTable):
    ...
    async def run(self) -> None:
        """
        Monitors SQS queues for ViewpointRequest and be able to process it. First, it will pick up a message from ViewpointRequest SQS.
        Then, it will download an image from S3 and save it to the local temp directory. Once that's completed, it will update the DynamoDB 
        ...
        while True:
            ...
            try:
                ...
                if len(messages) == 0:
                    sleep(5) # sleep for 5 seconds and retry
                    ...
                    if message_viewpoint_status == ViewpointStatus.REQUESTED:
                        ...
                        while(retry_count < 3):
                            try:
                                ...
                                self.logger.info(f"Successfully download {message_bucket_name}/{message_object_key} to {str(local_object_path.absolute())}. This Viewpoint is now READY!")
                                ...
                                if err.response["Error"]["Code"] == "404":
                                    ...
                                    internal_err = f"You do not have permission to access {message_bucket_name} bucket! Error={err}"
                                    self.logger.error(internal_err)
                        ...
                        self.logger.error(f"Cannot process this viewpoint request - {message_viewpoint_id} due to the incorrect Viewpoint Status {message_viewpoint_status}!")
                        continue
