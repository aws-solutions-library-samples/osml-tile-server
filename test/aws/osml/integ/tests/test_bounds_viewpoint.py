...
def get_bounds_viewpoint(self, viewpoint_id: str) -> bool:
    ...
    try:
        ...
        with open(f"integ/data/{self.image_type}/test_{self.image_type}_bounds.json", "r") as output_json:
...
def get_bounds_viewpoint_unhappy(self, viewpoint_id: str) -> bool:
    ...
    try:
        ...
        assert "Cannot view ViewpointApiNames.BOUNDS for this image since this has already been deleted." in response_data["detail"]
        return True
    except Exception as err:
        self.logger.error(traceback.print_exception(err))
        return False
