...
def get_statistics_viewpoint(self, viewpoint_id: str) -> bool:
    ...
    try:
        ...
        with open(f"integ/data/{self.image_type}/test_{self.image_type}_statistics.json", "r") as output_json:
            ...
            assert response_data["image_statistics"]["geoTransform"] == expected_json_result["image_statistics"]["geoTransform"]
            assert response_data["image_statistics"]["cornerCoordinates"] == expected_json_result["image_statistics"]["cornerCoordinates"]
...
def get_statistics_viewpoint_unhappy(self, viewpoint_id: str) -> bool:
    ...
    try:
        ...
        assert "Cannot view ViewpointApiNames.STATISTICS for this image since this has already been deleted." in response_data["detail"]
        return True
    except Exception as err:
        self.logger.error(traceback.print_exception(err))
        return False
